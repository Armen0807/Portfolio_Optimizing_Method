...
class MeanVarianceBacktestStrategy(Strategy):
    ...
    def __init__(self, **kwargs):
        ...
        self.config = MeanVarianceConfig(**self.config)
        ...
        if self.last_optimization_date is None or self.should_rebalance(self.last_optimization_date, current_date):
            ...
            if len(historical_data) == len(self.instruments):
                ...
                if not returns_df.empty:
                    ...
                    self.logger.info(f"Optimal weights: {self.optimal_weights}")
    ...
    def on_start(self):
        self.logger.info("Mean Variance Optimization Strategy Started")
        if not self.config.assets:
            self.logger.error("No assets specified in the configuration.")

    def on_stop(self):
        self.logger.info("Mean Variance Optimization Strategy Stopped")
